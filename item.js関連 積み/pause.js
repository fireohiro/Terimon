import {itemEvent, setupItemMenu, useItem, createItemButtons } from './item.js';
...
export function createPause(scene,gameStatus,playerStatus,config,friends,itemList,gearList){
    // setupItemMenu(scene, config, gameStatus);
    ...
    createItemButtons(scene, config, itemList);
    // playerStatus,,gameStatus,friends
}
...
async function menuBar(scene,playerStatus,config,gameStatus){
    ...
    itemtext.setInteractive().on('pointerdown',()=>{
        ...
        // console.log('Before calling itemEvent:', gameStatus);
        itemEvent(gameStatus);
        // console.log('After calling itemEvent:', gameStatus);
    });
