...
export async function saveGame(scene,playerStatus,config,gameStatus,friend1Status,friend2Status,friend3Status){
    ...
    const saveback = scene.add.rectangle(config.width * 0.3 + 10, 2, saveWidth, saveHeight, 0xFFFFFF, 0.8);
    saveback.setStrokeStyle(2,0x000000); // 緑枠

    //テキスト（ボタンを設定）の例
    const savetext = scene.add.text(config.width * 0.3 + 10 + saveWidth/2,saveHeight / -2 - 50,'はい',{fontSize:'18px'});
    const backtext = scene.add.text(config.width * 0.3 + 10 + saveWidth/2,saveHeight / -2,'いいえ',{fontSize:'18px'});
    //クリックイベント
    ...
    saveContainer = scene.add.container(0,0,[saveback,savetext,backtext]);
